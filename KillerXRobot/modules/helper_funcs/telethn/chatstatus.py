from KillerXRobot.modules.helper_funcs.telethn import HIGHER_AUTH, telethn
from KillerXRobot import SUPPORT_USERS, SARDEGNA_USERS, WHITELIST_USERS
...
async def KillerX_is_admin(chat_id: int):
    status = False
    KillerX = await telethn.get_me()
    async for user in telethn.iter_participants(
            chat_id, filter=ChannelParticipantsAdmins):
        if KillerX.id == user.id:
            status = True
            break
    return status
